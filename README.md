...
1. Explain the difference between `RDBMS` and `SQL`.
  - RDBMS is a management system for databases, such as Sqlite3, and SQL is a query language used to communicate with an RDBMS.
1. Why do tables need a `primary key`?
  - Tables need a primary key because each item in that table, say its a table for projects, needs to be uniquely identified, as well it needs a way to connect with other tables, and to do that we would link a foreign key to the primary key.
1. What is the name given to a table column that references the primary key on another table.
  - Foreign key
1. What do we need in order to have a _many to many_ relationship between two tables.
  - We would need a third table to act as a bridge between the two tables.
